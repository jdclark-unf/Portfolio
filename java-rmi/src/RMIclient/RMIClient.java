...
class MultipleClient extends Thread {
	...
	public void run(){
		...
		try {
            ...
            RMIProcessorInterface cp = (RMIProcessorInterface) registry.lookup("cp");
			...
			String outputString = cp.run(Integer.toString(choice));
			if(printOutput){
		...
		// finally, end Threads and decrement runningThreads
		finally {
			if (printOutput){
				System.out.println("\nClosing Thread\n");
			}
			// Decrease runningThreads count
